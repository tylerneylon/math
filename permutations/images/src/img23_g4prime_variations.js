/* img23_g4prime_variations.js
 *
 * Show, all in one spot, different key perspectives of G'_4, and attempt to
 * show how they relate to each other through animation.
 *
 */
...
function renderPtMap(artist, ptMap) {
    ...
    for (const edge of edges) {
        const edgeElt = artist.addLine(edge.from, edge.to, edgeStyle);
        push(edge.from, 'edgeElts', edgeElt);
        push(edge.to,   'edgeElts', edgeElt);
    }
}
...
function dropNonPrimeEdges(ptMap) {
    for (const [permStr, pt] of Object.entries(ptMap)) {
        for (let i = 0; i < pt.edgeElts.length; i++) {
            const edge    = pt.edges[i];
            const edgeElt = pt.edgeElts[i];
            const transp  = edge.transposition;
            const tDiff   = Math.abs(parseInt(transp[1]) - parseInt(transp[2]));

            if (tDiff !== 1) {
                edgeElt.baseColor = 'transparent';
                edgeElt.setAttribute('stroke', 'transparent');
            }
        }
    }
}
