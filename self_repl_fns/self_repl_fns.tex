...
\textbf{Proof} \(\;\) The forward direction --- that the left-hand side
implies the right --- has already been justified by
(\ref{eq:nonpl_base_case}) and (\ref{eq:h_reln}). Note that these two
equations are each expanded upon in theorems 1 and 2.

To verify the other direction, it suffices to show that

\begin{equation}f(x) = f_S(t_1(x)) = f(t_1(x)) + f(t_1(x) - 2);\label{eq:thm3_pf}\end{equation}

that equation is enough to ensure that definition (\ref{eq:exact_defn})
is satisfied.

Suppose \(x=1.CE_3 \in (1,2)\), where \(C \in \{0, 1, 2\}\) and \(E\)
does not have an all-2 tail; let \(x' = t_1(x)\). The argument can be
split into three caseson the value of \(C\).
