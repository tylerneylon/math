...
could be replaced by curves with the shapes \(x^2\) and \(1-x^2\) on
\([0, 1]\), as illustrated below.

\begin{figure}[htbp]
\centering
\includegraphics{images/pdfs/nonlinear_ramps.pdf}
\caption{An example of nonlinear ramp functions using \(x^2\) to
determine the edge shapes}
\end{figure}

Given any function \(f:[0,1]\to [0,1]\) with \(f(0) = 0\) and \(f(1)=1\)
the general form of this case is the function
