...
TODO This is accidentally the inverse of \(t_1\). Fix this definition
and the corresponding image below; check for any other residual errors.
...
\([0, 3]\). Define \(r_L = f\restrict [0, 1]\) and
...
\(f\) is also exactly self-replicating. I'll use definition
...
Recall that the notation \(1 + 0.\zerotwo^k1\lstar\) indicates a union
of closed intervals. In the next theorem, the notation
...
Let \(A_k = 1 + 0.\zerotwo^k1\lstar\). Figure~\ref{fig:nonpl_process}
illustrates \(A_k\) for low values of \(k\).
...
If \(x\in A_k\), then either \(x=0.\zerotwo^k1\ldots\) or
\(x=0.\zerotwo^k2\). Suppose \(x_j\in A_j\) and \(x_k\in A_k\), where
\(j < k\). Then either the \((j+1)^\text{th}\) digit of \(x_j\) is 1, or
the \((j+1)^\text{th}\) digit of \(x_j\) is 2 and all subsequent digits
are 0. In either case, \(x_j\ne x_k\) since the \((j+1)^\text{th}\)
digit of \(x_k\) can't be 1 and the \((k+1)^\text{th}\) digit of \(x_k\)
must be 1. Thus all of the \(A_k\) sets are pairwise disjoint.
...
\(k\)? The choice is still not arbitrary as the values remain related.
Note that, given \(x\), there is some \(k\) with \(x\in A_k\) iff the
base-3 expansion of \(x\) contains a 1 or if it ends with the tail
\(\overline{0}\), excluding \(x=1\) and \(x=2\). This can be written
...
expansion with \(x=1.E_3\). Then \(f_S(x+\{0,2\}) = y\) and, by applying
\(t_1\), \(f(x') = y\) for both \(x' = (x+2)/3\) and \(x' = (x+4)/3\).
In expansion notation, we can write these last two equations as
\(x' = x/3 + 2/3 = 0.1E_3 + 0.2_3 = 1.0E_3\) and as
\(x' = x/3 + 4/3 = 0.1E_3 + 1.1_3 = 1.2E_3\). We can summarize this
reasoning as

\begin{equation}f(1.\zerotwo E_3) = f(1.E_3).\label{eq:h_reln}\end{equation}
...
\textbf{Theorem 2} \(\;\) \emph{Suppose \(x=1.E_3\) and \(y=1.F_3\),
where both expansions \(E\) and \(F\) exlude 1 from their range, which
we could express as \(x,y\in 1.\zerotwo^\infty_3\). Then}

\[\tail(x) = \tail(y) \quad\Rightarrow\quad f(x) = f(y).\]

\textbf{Proof} \(\;\) If \(\tail(x) = \tail(y)\), then there exist
...
Although I haven't proven it yet, it turns out that theorems 1 and 2
capture \emph{all} of the restrictions needed for \(f\) to be exactly
self-replicating. We can capture this by working with three arbitrary
functions: \(r_L : [0,1] \to \R\)
...
\(A_k\). Let \(b\) be an expansion with \(v(b) = x-1\). Then
...
\(A_k\). Then \(f_S(x') = y\) for both \(x' = x\) and \(x' = x + 2\).
