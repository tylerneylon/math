...
normal-like curves that are reflections of each other.}
\end{figure}
...
leaf point density is provided around an outer ellipse.}
\end{figure}
...
addition of indicator functions of intervals.}
\end{figure}
...
\subsection{Nonlinear ramps}\label{nonlinear-ramps}
...
on the bottom is a continuous but non-peak-monotonic function.}
\end{figure}

\subsection{Non-plateau functions}\label{non-plateau-functions}

The ramp functions \(K_{a,b,c,d}\) so far all have the constant value 1
on the middle interval \([b, c]\). This requires the ramps on intervals
\([a, b]\) and \([c, d]\) to sum to 1. What happens if we relax this
condition?
...
In other words, \(f_L\) and \(f_R\) are possibly-reflected shifts of
each other, and \(f_S\) can be transformed to recover exactly \(f_L\).
If a

So far we haven't formally stated the similarity relationship between
