...
Given an expansion \(E\), let
\(\text{tail}(E) = \big\{\text{expansion }\eta \;\big|\; \exists\, j, k: E(j + m) = \eta(k + m) \,\forall\, m \ge 0\big\}\).
Conceptually, \(\text{tail}(E)\) is the set of all numbers in \([0, 1]\)
with the same final sequence of base-3 digits as \(E\), ignoring any
finite prefix of either expansion. For example,
\(x=0.21021\overline{011}_3\) and \(y=0.001\overline{011}_3\) have
\(\text{tail}(x) = \text{tail}(y)\).

Suppose that \(f(x) = y\) for some \(x\in (1,2)\) and not in any set
\(A_k\). Let \(E\) be the expansion with \(x=1.E_3\). Then
\(f_S(x+\{0,2\}) = y\) and, by applying \(t_1\), \(f(x') = y\) for
\(x' = (x+2)/3\) and \(x' = (x+4)/3\). In expansion notation, we can
write these last two equations as
