...
self-replicating. We can see this by working with three functions:
\(r_L : [0,1] \to \R\), \(r_R : [0,1] \to \R\), and
\(h : (1,2) - G \to \R\); with the restriction that
\(\tail(x) = \tail(y) \Rightarrow h(x) = h(y)\). Theorem 1 allows us to
build a function \(f:[0,3]\to\R\) based on \(r_L,\) \(r_R,\) and \(h.\)
