...
From here on, I'll more formally use the word \emph{expansion} to
indicate a function \(b:\mathbb{N}_{\ge 1}\to \{0, 1, 2\}\) which
denotes the value \(v(b) = \sum_{k\ge 1}b(k)/3^k\). Given an expansion
\(b\), let
\(\text{tail}(b) = \{\text{expansion }\beta \;|\; \exists\, j, k: b(j + m) = \beta(k + m) \,\forall\, m \ge 0\}\).
Conceptually, \(\text{tail}(b)\) is the set of all numbers in \([0, 1]\)
with the same final sequence of base-3 digits as \(b\), ignoring any
