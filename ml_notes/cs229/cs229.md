...
I'll use $K'$ for the function version and $K$ for the matrix version,
both defined in a moment.
The function is defined as

$$K'(x, z) = \exp\left(-\frac{||x-z||^2}{2\sigma^2}\right).$$

Suppose we have a set of points $\{x^{(1)}, \ldots, x^{(m)}\};$
define matrix $K$ by letting
$K_{ij} = K'(x^{(i)}, x^{(j)}).$
By Mercer's theorem,
our job is to 
show that, for any vector $z,$ $z^TKz \ge 0.$

NOTE The solution below is not correct. I'm working on updating it
with a correct version.

Let's proceed as follows, using $c = -\frac{1}{2\sigma^2}:$

$$\begin{aligned}
z^TKz & = z_i K_{ij} z_j \\
      & = z_i z_j \exp(c||x\toi - x\toj||^2) \\
      & = z_i z_j \exp\bigg(c\big(||x\toi||^2 + ||x\toj||^2 - 2\langle x\toi,
           x\toj \rangle\big) \bigg) \\
      & = z_i e^{c||x\toi||^2} z_j e^{c||x\toj||^2}
              \exp\left(-2c\sum_k \big(x_k\toi x_k\toj\big)\right) \\
      & = z_i e^{c||x\toi||^2} z_j e^{c||x\toj||^2}
              \prod_k e^{-2cx_k\toi} e^{-2cx_k\toj} \\
      & = \left(z_i e^{c||x\toi||^2} \prod_k e^{-2cx_k\toi}\right)
          \left(z_j e^{c||x\toj||^2} \prod_k e^{-2cx_k\toj}\right) \\
      & = \left(z_i e^{c||x\toi||^2} \prod_k e^{-2cx_k\toi}\right)^2 \\
      & \ge 0.
\end{aligned}$$
